import { LocalMove } from '../lib/contracts'
            ...
            [0, 1, 2].map((x) => {
              ...
              return (
                ...
                >
                  ...
                  {isCurrentSelection && !isMyCell && (
                    <span className={`opacity-50 ${isPlayer1 ? 'text-cyber-purple' : 'text-cyber-cyan'}`}>
                      {isPlayer1 ? '✕' : '○'}
                    </span>
                  )}
                  
                  {/* Phantom/hidden cell - show lock icon */}
                  {!isMyCell && !isCurrentSelection && (
...
// Mini board for previews (shows only player's known moves)
